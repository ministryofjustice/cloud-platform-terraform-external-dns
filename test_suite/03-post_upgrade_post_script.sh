...
upsert_record ext-dns-ing-2 $HZ_2_ADDR $HZ_2_ID $NAMESPACE

echo "Checking DELETE by deleting APEX ingress in hosted zone 2"
delete_record ext-dns-ing-2 $HZ_2_ADDR $HZ_2_ID $NAMESPACE

echo "Checking UPSERT by changing aws-weight annotation for SUBDOMAIN in Hosted Zone 2"
upsert_record ext-dns-ing-4 $HZ_3_ADDR $HZ_2_ID $NAMESPACE 98

echo "Checking DELETE by deleting SUBDOMAIN ingress"
delete_record ext-dns-ing-4 $HZ_2_ADDR $HZ_2_ID $NAMESPACE 98
...
create_record fixtures/apex-post-ing-5.yaml $HZ_3_ADDR $HZ_3_ID $NAMESPACE

echo "CREATE new subdomain record..."
create_record fixtures/sub-post-ing-6.yaml $HZ_3_ADDR $HZ_3_ID $NAMESPACE

echo "Checking UPSERT by changing aws-weight annotation for APEX in Hosted Zone 3"
upsert_record ext-dns-ing-6 $HZ_3_ADDR $HZ_3_ID $NAMESPACE

echo "Checking UPSERT by changing aws-weight annotation for SUBDOMAIN in Hosted Zone 3"
upsert_record ext-dns-ing-5 $HZ_3_ADDR $HZ_3_ID $NAMESPACE 98

echo "Checking DELETE by deleting APEX ingress in hosted zone 3"
delete_record ext-dns-ing-5 $HZ_3_ADDR $HZ_3_ID $NAMESPACE 98

echo "Checking DELETE by deleting SUBDOMAIN ingress post upgrade in hosted zone 3"
delete_record ext-dns-ing-6 $HZ_3_ADDR $HZ_3_ID $NAMESPACE
